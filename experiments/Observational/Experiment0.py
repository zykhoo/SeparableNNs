...
for i in range(1,5):
  ...
  for ini in initialcon: 
    ...
    for i in tqdm(range(1, wholemat.shape[0])):
      ...
      if i in [10,20,30,40,50]: #[1,2,3,4,5,10,20,30,40,50,100,200,300,400]:
        ...
        torch.save(net.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"obsNet%s" %i,seed,ini,0,0))
        starttime = time.time() 
        results = train(net,name="%s/%s/%s_%s_obsNet%s" %(loc,s,seed,ini,i),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=expmat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
        net, epochs = results[0], results[1]
        PINNtraintime = time.time()-starttime
        torch.save(net.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"obsNet%s" %i,seed,ini,epochs,PINNtraintime))
