...
for i in range(20):
  ...
  for ini in initialcon: 
    ...
    torch.save(net.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0))

    snet = Net(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    snet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0)))
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-1.00",seed,ini,0,0))
    results = train(snet,name="%s/%s/%s_%s_PINN_softsep-1.00" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=1.00)
    snet, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-1.00",seed,ini,epochs,PINNtraintime))

    snet = Net(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    snet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0)))
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-2.00",seed,ini,0,0))
    results = train(snet,name="%s/%s/%s_%s_PINN_softsep-2.00" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=2.00)
    snet, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-2.00",seed,ini,epochs,PINNtraintime))

    snet = Net(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    snet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0)))
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-4.00",seed,ini,0,0))
    results = train(snet,name="%s/%s/%s_%s_PINN_softsep-4.00" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=4.00)
    snet, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-4.00",seed,ini,epochs,PINNtraintime))

    snet = Net(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    snet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0)))
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-0.25",seed,ini,0,0))
    results = train(snet,name="%s/%s/%s_%s_PINN_softsep-0.25" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0.25)
    snet, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-0.25",seed,ini,epochs,PINNtraintime))

    snet = Net(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    snet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep",seed,ini,0,0)))
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-0.50",seed,ini,0,0))
    results = train(snet,name="%s/%s/%s_%s_PINN_softsep-0.50" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0.50)
    snet, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"PINN_softsep-0.50",seed,ini,epochs,PINNtraintime))
