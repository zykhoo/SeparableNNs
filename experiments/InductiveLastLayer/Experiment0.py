...
def relu(x):
    return torch.max(torch.zeros(x.shape).to(device),x)
...
class sepNet5(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("hl2", x.shape)
        x = torch.squeeze(self.output_layer(x), dim = 2)
        x = relu(self.output1_layer(torch.transpose(x,0,1)))
        # print(x)
        return x
...
def weights_copy(newNet, sepnet):
  with torch.no_grad():	
    newNet.hidden_layer_1.weights.copy_(sepnet.hidden_layer_1.weights)
    newNet.hidden_layer_1.bias.copy_(sepnet.hidden_layer_1.bias)
    newNet.hidden_layer_2.weights.copy_(sepnet.hidden_layer_2.weights)
    newNet.hidden_layer_2.bias.copy_(sepnet.hidden_layer_2.bias)
    newNet.output_layer.weights.copy_(sepnet.output_layer.weights)
    newNet.output_layer.bias.copy_(sepnet.output_layer.bias)
  return newNet
...
for i in range(20):
  ...
  for ini in initialcon: 
    ...
    SepNet = sepNet1(sys.sepshape1,sys.sepshape2,sys.sepshape3,sys.sepshape4) 
    starttime = time.time() 
    print("training SepNet")
    SepNet = weights_copy(SepNet, sepnet)
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet1",seed,ini,0,0))
    SepNet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet1",seed,ini,0,0)))
    starttime = time.time() 
    results = train(SepNet, name="%s/%s/%s_%s_SepNet1" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    SepNet, epochs = results[0], results[1]
    septraintime = time.time()-starttime
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet1",seed,ini,epochs,septraintime))
    

    SepNet = sepNet2(sys.sepshape1,sys.sepshape2,sys.sepshape3,sys.sepshape4) 
    starttime = time.time() 
    print("training SepNet")
    SepNet = weights_copy(SepNet, sepnet)
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet2",seed,ini,0,0))
    SepNet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet2",seed,ini,0,0)))
    starttime = time.time() 
    results = train(SepNet, name="%s/%s/%s_%s_SepNet2" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    SepNet, epochs = results[0], results[1]
    septraintime = time.time()-starttime
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet2",seed,ini,epochs,septraintime))
    

    SepNet = sepNet3(sys.sepshape1,sys.sepshape2,sys.sepshape3,sys.sepshape4) 
    starttime = time.time() 
    print("training SepNet")
    SepNet = weights_copy(SepNet, sepnet)
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet3",seed,ini,0,0))
    SepNet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet3",seed,ini,0,0)))
    starttime = time.time() 
    results = train(SepNet, name="%s/%s/%s_%s_SepNet3" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    SepNet, epochs = results[0], results[1]
    septraintime = time.time()-starttime
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet3",seed,ini,epochs,septraintime))
    
    SepNet = sepNet4(sys.sepshape1,sys.sepshape2,sys.sepshape3,sys.sepshape4) 
    starttime = time.time() 
    print("training SepNet")
    SepNet = weights_copy(SepNet, sepnet)
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet4",seed,ini,0,0))
    SepNet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet4",seed,ini,0,0)))
    starttime = time.time() 
    results = train(SepNet, name="%s/%s/%s_%s_SepNet4" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    SepNet, epochs = results[0], results[1]
    septraintime = time.time()-starttime
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet4",seed,ini,epochs,septraintime))


    SepNet = sepNet5(sys.sepshape1,sys.sepshape2,sys.sepshape3,sys.sepshape4) 
    starttime = time.time() 
    print("training SepNet")
    SepNet = weights_copy(SepNet, sepnet)
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet5",seed,ini,0,0))
    SepNet.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet5",seed,ini,0,0)))
    starttime = time.time() 
    results = train(SepNet, name="%s/%s/%s_%s_SepNet5" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    SepNet, epochs = results[0], results[1]
    septraintime = time.time()-starttime
    torch.save(SepNet.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"SepNet5",seed,ini,epochs,septraintime))
